...
namespace wrench {
    ...
    BatchService::BatchService(std::string hostname,
        ...
        if (((cores_per_host != num_cores_available)) or
            ((ram_per_host != ram_available))) {
            throw std::invalid_argument(
        ...
        this->ram_per_node = ram_per_host;
...
//                  workflow_job->getName().c_str());
        ...
        std::map<std::string, std::tuple<unsigned long, double>> resources = this->scheduleOnHosts(
                ...
                num_nodes_asked_for, cores_per_node_asked_for, this->ram_per_node);
