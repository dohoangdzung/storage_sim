...
//              run();
    ...
    void Workflow::loadFromDAX(const std::string &filename, const std::string &reference_flop_rate,
      ...
      for (pugi::xml_node job = dag.child("job"); job; job = job.next_sibling("job")) {
        ...
        double flops = UnitParser::parse_compute_speed(job.attribute("flop").value());
        double memory_required = UnitParser::parse_size(job.attribute("memory").value());
        int num_procs = 1;
        ...
        if (flops > 0) {
            task = this->addTask(id, flops, 1, num_procs, 1.0, memory_required);
        } else {
            task = this->addTask(id, runtime * flop_rate, 1, num_procs, 1.0, memory_required);
        }
